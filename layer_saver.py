...
class LayerSaver:
    ...
    def saveLayer(self):
        # show the dialog
        self.saverDlg.set_target_directory(
            QSettings().value('plugins/layersaver/path', QgsProject.instance().homePath()))
        self.saverDlg.show()
        ...
        if result:
            ...
            for layerid in self.saverDlg.layers():
                ...
                self.iface.messageBar().pushInfo(self.tr('LayerSaver'),
                                                 self.tr('Exported {0} layers').format(len(exporter.traversed_layers)))

    def loadLayer(self):
        # show the dialog
        self.loaderDlg.set_target_directory(
            QSettings().value('plugins/layersaver/path', QgsProject.instance().homePath()))
        self.loaderDlg.show()
